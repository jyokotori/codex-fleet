...
pub async fn list_docker_configs(
    ...
    let rows = sqlx::query!(
        "SELECT id, name, port_mappings, env_vars, init_script, created_at, updated_at FROM docker_configs ORDER BY created_at DESC"
    )
    .fetch_all(&state.db)
    .await?;
...
pub async fn create_docker_config(
    ...
    sqlx::query!(
        "INSERT INTO docker_configs (id, name, port_mappings, env_vars, init_script, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        id, req.name, port_mappings_str, env_vars_str, init_script, now, now
    )
    .execute(&state.db)
    .await?;
...
pub async fn update_docker_config(
    ...
    let existing = sqlx::query!(
        "SELECT id, name, port_mappings, env_vars, init_script, created_at, updated_at FROM docker_configs WHERE id = $1",
        id
    )
    ...
    sqlx::query!(
        "UPDATE docker_configs SET name=$1, port_mappings=$2, env_vars=$3, init_script=$4, updated_at=$5 WHERE id=$6",
        name, port_mappings_str, env_vars_str, init_script, now, id
    )
    .execute(&state.db)
    .await?;
