...
use crate::{
    ...
    ssh::client::{ensure_ssh_key, read_public_key, SshClientPool},
    AppState,
};
...
pub async fn create_server(
    ...
    let passwordless_ok = SshClientPool::connect_passwordless(&req.ip, port, &req.username, &key_path)
        .await
        .is_ok();
...
pub async fn test_server_connection(
    ...
    let result =
        SshClientPool::connect_passwordless(&server.ip, server.port as u16, &server.username, &key_path)
            .await;
