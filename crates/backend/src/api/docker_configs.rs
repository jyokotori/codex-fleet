...
pub async fn list_docker_configs(
    ...
    let configs = rows
        ...
        .map(|r| DockerConfig {
            ...
            volume_mappings: serde_json::from_str(&r.volume_mappings).unwrap_or(serde_json::json!([])),
...
pub async fn update_docker_config(
    ...
    let port_mappings = req
        .port_mappings
        .unwrap_or_else(|| serde_json::from_str(&existing.port_mappings).unwrap_or(serde_json::json!([])));
    let env_vars = req
        .env_vars
        .unwrap_or_else(|| serde_json::from_str(&existing.env_vars).unwrap_or(serde_json::json!([])));
    let volume_mappings = req
        .volume_mappings
        .unwrap_or_else(|| serde_json::from_str(&existing.volume_mappings).unwrap_or(serde_json::json!([])));
