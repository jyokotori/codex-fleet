...
钉钉凭据 (`app_key`、`app_secret`、`robot_code`) 通过管理员后台 **配置 → 钉钉** 在数据库中维护，不再从 `.env` 读取。在配置并启用之前，用户管理页的"同步钉钉用户"按钮和通知页的"钉钉"通知类型都会被隐藏。

钉钉用户同步使用的新建用户默认密码不会持久化。管理员每次在用户管理页启动同步任务时，在弹窗中输入该默认密码。

每个用户的 API 访问令牌在 **配置 → API 访问令牌** 中管理。每个用户可生成（或重新生成）一个令牌，并通过 `Authorization: Bearer <token>` 调用系统中的任意 API。携带该令牌的请求以该用户身份处理，沿用其角色与权限。重新生成会立即让旧令牌失效。
...
- 配置钉钉通知；通知记录中不保存钉钉凭据，凭据统一在 **配置** 管理（仅管理员可见）。
