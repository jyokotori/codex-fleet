...
Plane Issue   ── PATCH state ──> In Progress
        ...
        │
        ▼ ④ 人工审核（requirements.rs；当前仅 work_item 路径，纯 Plane 任务未接入）
        ├─ human_approved：Plane 当前为 Human Review → Done + comment
        └─ human_rejected：Plane 当前为 Human Review → Review Failed + comment
           （若 Plane 状态已不在 Human Review，仅加 comment，不改状态）
```
...
迁移文件：`crates/backend/migrations/004_plane_integration.sql`
...
6. **下发**：`dispatch_task_for_agent(..., work_item_id=None, ...)` —— Plane 任务不创建 work_item。
7. **成功后**：
...
文件：`crates/runtime_agent/src/api/requirements.rs`
触发点：work_item 状态切换为 `human_approved` / `human_rejected`。

同样通过 `JOIN plane_workspaces` 拿凭证；逻辑：
- `human_approved` + Plane 当前 `Human Review` → `Done` + comment
- `human_rejected` + Plane 当前 `Human Review` → `Review Failed` + comment
- 否则仅 comment，不改状态

**已知缺口**：纯 Plane 任务（`plane_dispatch_one` 下发时 `work_item_id=None`）不会经过 work_item 审核流，所以 5.1 把 Plane 置为 Human Review 之后，codex-fleet 端没有 UI 闭环把它推进到 Done / Review Failed。该路径当前仍是预备/死代码，需要后续把 Plane 任务也串入 work_item 审核列表才会激活。
...
1. **dispatch 路径与 work_item 解耦**：Plane 任务 `work_item_id=None`，不走 work_item 审核流水线；requirements.rs 中的 Plane 回写只在有 work_item 时才触发，因此"人工审核→Done/Review Failed"闭环尚未自动闭合。
2. **plane_dispatch_one 没有 agent 锁**：并发下多个 pending plane_task 可能同时命中同一 agent，依赖下游幂等。
3. **`agent_completed` 无条件**置 Human Review，而 `agent_failed` 要求当前 In Progress 才改状态——两个分支逻辑不对称。
4. **state 名称硬编码**（`Todo / In Progress / Human Review / Review Failed / Done`）散落在 scheduler、tasks、requirements、webhooks；改名或多语言 Plane 项目不可用。
...
- 人工审核回写：`crates/runtime_agent/src/api/requirements.rs` —— Plane write-back 段
- 路由注册：`crates/runtime_agent/src/lib.rs`
- 迁移：`crates/backend/migrations/004_plane_integration.sql`
- 前端页面：`frontend/src/pages/PlaneIntegration.tsx`
